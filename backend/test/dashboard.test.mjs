...
test("dashboard API applies one filter to every dataset and preserves payout samples", async () => {
  ...
  try {
    ...
    async function heatmap(query) {
      const response = await fetch(`${base}/api/dashboard/destination-heatmap?${query}`);
      return { status: response.status, body: await response.json() };
    }
    ...
    assert.equal(map.merchantDiversity.length, summary.uniqueMerchants);
    assert.ok(map.merchantDiversity.every((row) => row.distinctMerchantCount === 1));
    assert.equal(Object.hasOwn(map, "destinationHeatmap"), false);
    assert.equal(JSON.stringify(all.body).includes("destinationLocation"), false);
    assert.equal(JSON.stringify(all.body).includes("0.1,0.2"), false);

    const destinations = await heatmap("period=week&category=all&asOf=2026-04-20");
    assert.equal(destinations.status, 200);
    assert.deepEqual(Object.keys(destinations.body), ["cells"]);
    assert.equal(destinations.body.cells.length, summary.observedDestinationAreas);
    assert.equal(destinations.body.cells.reduce((sum, row) => sum + row.count, 0), 3);
    assert.deepEqual(destinations.body.cells[0], { location: { type: "Point", coordinates: [0.1, 0.2] }, count: 2 });
    assert.ok(destinations.body.cells.every((cell) => Object.keys(cell).sort().join(",") === "count,location"));
    assert.equal(/address|merchantId|payout|notes|exact/i.test(JSON.stringify(destinations.body)), false);
    ...
    assert.equal(grocery.map.merchantDiversity[0].distinctMerchantCount, 1);
    assert.deepEqual((await heatmap("period=week&category=grocery&asOf=2026-04-20")).body.cells,
      [{ location: { type: "Point", coordinates: [0.3, 0.4] }, count: 1 }]);
    assert.deepEqual((await heatmap("period=week&category=restaurant&asOf=2026-04-20")).body.cells,
      [{ location: { type: "Point", coordinates: [0.1, 0.2] }, count: 2 }]);
    assert.deepEqual((await heatmap("period=week&category=retail&asOf=2026-04-20")).body.cells, []);
    ...
    const monthDestinations = (await heatmap("period=month&category=all&asOf=2026-04-20")).body.cells;
    assert.equal(monthDestinations.reduce((sum, row) => sum + row.count, 0), 4);
    assert.ok(monthDestinations.some((row) => row.location.coordinates.join(",") === "0.5,0.6"));
